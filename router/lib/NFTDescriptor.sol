...
library NFTDescriptor {
    ...
    function tickToDecimalString(
        ...
        if (tick == (SwapMath.MIN_TICK / tickSpacing) * tickSpacing) {
            return !flipRatio ? 'MIN' : 'MAX';
        } else if (tick == (SwapMath.MAX_TICK / tickSpacing) * tickSpacing) {
    ...
    function adjustForDecimalPrecision(
        ...
        if (difference > 0 && difference <= 18) {
            if (baseTokenDecimals > quoteTokenDecimals) {
                ...
                if (difference % 2 == 1) {
                    adjustedSqrtRatioX96 = SwapMath.mulDiv(adjustedSqrtRatioX96, sqrt10X128, 1 << 128);
                }
            } else {
                adjustedSqrtRatioX96 = sqrtRatioX96.div(10**(difference.div(2)));
                if (difference % 2 == 1) {
                    adjustedSqrtRatioX96 = SwapMath.mulDiv(adjustedSqrtRatioX96, 1 << 128, sqrt10X128);
                }
            }
        } else {
    ...
    function fixedPointToDecimalString(
        ...
        uint256 value = SwapMath.mulDiv(adjustedSqrtRatioX96, adjustedSqrtRatioX96, 1 << 64);
        ...
        if (priceBelow1) {
            ...
            value = SwapMath.mulDiv(value, 10**44, 1 << 128);
        } else {
            // leave precision for 4 decimal places + 1 place for rounding
            value = SwapMath.mulDiv(value, 10**5, 1 << 128);
        }
